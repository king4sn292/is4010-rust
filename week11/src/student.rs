...
impl Student {
    ...
    pub fn calculate_gpa(&self) -> f32 {
        let total_credits: u16 = self.grades.iter().map(|grade| grade.credits).sum();
        if total_credits == 0 {
            return 0.0;
        }

        let total_quality_points: f32 = self.grades.iter().map(|grade| grade.quality_points()).sum();
...
impl StudentDatabase {
    ...
    pub fn average_gpa(&self) -> f32 {
        ...
        let total: f32 = self.students.values().map(|student| student.calculate_gpa()).sum();
        total / count as f32
    }
