...
pub fn analyze_text(text: &str) -> (usize, f64, String) {
    ...
    let average_word_length = total_len as f64 / count as f64;
    ...
    (count, average_word_length, longest_word)
}
...
pub fn parse_positive_number(input: &str) -> Result<i32, ParseError> {
    let value: i32 = input.trim().parse().map_err(|_| ParseError::NotANumber)?;
    if value > 0 {
        Ok(value)
    } else {
